...
function loadImage(i, t) {
  ...
  for (let e = 1; e <= t; e += 2) {
    o += '<div class="images">\n                            <img src="'.concat(i[e].images.fixed_width.url, '"/>\n                            <img src="').concat(i[e + 1].images.fixed_width.url, '"/>\n                        </div>');
  }
...
xhr.onreadystatechange = function () {
  if (this.readyState == 4 && this.status == 200) {
    ...
    loadImage(i.data, 7), showBtn.addEventListener('click', () => {
      showClicked = !0, showBtn.style.display = 'none', loadImage(i.data, 48);
    });
  }
};
...
function link(e) {
  showSelection.style.display = 'inline-block', img.src = e;
}

xhr.open('GET', url, !0), xhr.send(), searchBtn.addEventListener('click', () => {
  const t = 'https://api.giphy.com/v1/gifs/search?q='.concat(gifSearch.value, '&rating=g&api_key=2FJgFS8i3VJXmBrVIKC6g35vMTbZcsiL');
  !(function _callee() {
    let e; let
      i;
    return regeneratorRuntime.async((_context) => {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return regeneratorRuntime.awrap(fetch(t));

          case 2:
            e = _context.sent;
            i = e.json();
            i.then((e) => {
              loadImage(e.data, 7), showBtn.addEventListener('click', () => {
                showClicked = !0, showBtn.style.display = 'none', loadImage(e.data, e.data.length - 2);
              });
...
}), gif.addEventListener('scroll', (e) => {
  e.target.scrollTop >= e.target.clientHeight / 2 && !0 !== showClicked ? showBtn.style.display = 'block' : showBtn.style.display = 'none';
}), showSelection.style.display = 'none', gif.addEventListener('click', (e) => {
  link(e.target.src);
});
const time = document.getElementById('js-time');
const date = document.getElementById('js-date');
const input = document.getElementById('js-getFile');
...
for (let e = 0; e < fullDate.length; e++) {
  e < 3 ? date.innerHTML += '<span>'.concat(fullDate[e], ' </span>') : time.innerHTML += '<span>'.concat(fullDate[e], '</span>');
}
...
textUsers.forEach((e, i) => {
  e.textContent.length >= 12 ? (lengthCut = '...', usernames[i].innerHTML = lengthCut) : e.textContent.length > 4 && (lengthCut = '@'.concat(e.textContent.substring(0, 4), '...'), usernames[i].innerHTML = lengthCut);
}), input.onchange = function () {
  const _input$files = _slicedToArray(input.files, 1);
  const e = _input$files[0];

  showSelection.style.display = 'inline-block', img.src = URL.createObjectURL(e), URL.revokeObjectURL(e);
};
...
function overlayCall(e) {
  e ? (overlay.classList.add('visible'), side.style.overflow = 'hidden') : (overlay.classList.remove('visible'), side.style.overflow = 'scroll');
}

const span = document.createElement('span');
personalInput.replaceWith(span), textarea.addEventListener('click', () => {
  sideBtn.hidden = !0, span.replaceWith(personalInput);
});
let scrollHeight;
let currentLength = 0;
textarea.addEventListener('input', (e) => {
  scrollHeight = textarea.scrollHeight, textarea.style.height = ''.concat(scrollHeight, 'px'), textarea.textLength === currentLength + 23 ? currentLength = textarea.textLength : currentLength >= textarea.textLength && (scrollHeight <= 46 ? textarea.style.height = '46px' : textarea.style.height = ''.concat(scrollHeight - 19, 'px')), e.target.value.length > 0 ? (personalBtn.style.pointerEvents = 'auto', personalBtn.style.background = '#088fc4', personalBtn.style.cursor = 'pointer') : (personalBtn.style.pointerEvents = 'none', personalBtn.style.background = '#8dd4f0');
});
let menuBoxNum;
body.addEventListener('click', (e) => {
  const i = new String(e.target.id);

  if (!isNaN(i[i.length - 1]) && i.slice(0, i.length - 1) === 'js-menu-btn') {
    const t = document.getElementById('js-menu-box'.concat(i[i.length - 1]));
    t.classList.add('visible'), overlayCall(!0), menuBoxNum = t;
  }
}), menuBtn.addEventListener('click', () => {
  menu.classList.add('visible'), overlayCall(!0);
}), overlay.addEventListener('click', () => {
  overlayCall(!1), replyBox.forEach((e) => {
    e === menuBoxNum && e.classList.remove('visible');
...
function Emoji(e) {
  ...
  this.options = e, this.trigger = this.options.trigger.map((e) => e.selector), this.insertInto = void 0;
  let a = '';
  let m = '';
  let n;
  let t = !1;
  const i = this.options.closeButton ? 370 : 350;

  this.lib = function () {
    const t = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : void 0;
    return {
      el: function el() {
        let e;
        let i;
        if (t) return t.nodeName ? [t] : (e = t, i = Object.prototype.toString.call(e), _typeof(e) === 'object' && /^\[object (HTMLCollection|NodeList|Object)\]$/.test(i) && typeof e.length === 'number' && (e.length === 0 || _typeof(e[0]) === 'object' && e[0].nodeType > 0) ? Array.from(t) : typeof t === 'string' || typeof t === 'STRING' ? Array.from(document.querySelectorAll(t)) : void 0);
      },
      on: function on(i, l) {
        const a = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : void 0;
        a ? this.el().forEach((e) => {
          e.addEventListener(i, (i) => {
            if (i.target.closest(a)) {
              ...
              if (Array.isArray(a)) {
                const o = i.target.outerHTML;

                const _t = a.findIndex((e) => o.includes(e.slice(1)));
          ...
          e.addEventListener(i, l.bind(e));
        });
      },
      css: function css(e) {
        const _this = this;

        const _loop = function _loop(_i2) {
          if (Object.hasOwnProperty.call(e, _i2)) {
            const _t2 = e[_i2];

            _this.el().forEach((e) => e.style[_i2] = _t2);
          }
        };
      ...
      attr: function attr(i) {
        const t = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : void 0;
        if (!t) return this.el()[0].getAttribute(i);
        this.el().forEach((e) => e.setAttribute(i, t));
      },
      removeAttr: function removeAttr(i) {
        this.el().forEach((e) => e.removeAttribute(i));
      },
      addClass: function addClass(i) {
        this.el().forEach((e) => e.classList.add(i));
      },
      removeClass: function removeClass(i) {
        this.el().forEach((e) => e.classList.remove(i));
      },
      slug: function slug(e) {
        return e.toLowerCase().replace(/[^\u00BF-\u1FFF\u2C00-\uD7FF\w]+|[\_]+/gi, '-').replace(/ +/g, '-');
      },
      remove: function remove(e) {
        this.el().forEach((e) => e.remove());
      },
      val: function val() {
        const i = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : void 0;
        let t;
        return void 0 === i ? this.el().forEach((e) => {
          t = e.value;
        }) : this.el().forEach((e) => {
          e.value = i;
        }), t;
      ...
      text: function text() {
        const i = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : void 0;
        if (void 0 === i) return t.innerText;
        this.el().forEach((e) => {
          e.innerText = i;
        });
      },
      html: function html() {
        const i = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : void 0;
        if (void 0 === i) return t.innerHTML;
        this.el().forEach((e) => {
          e.innerHTML = i;
        });
      },
  ...
  var g = {
    styles: function styles() {
      const e = '\n                <style>\n                    .fg-emoji-container {\n                        width: '.concat(i, 'px;\n                        height: 400px;\n                        border-radius: 5px;\n                        box-shadow: 0px 3px 20px 0px rgba(0, 0, 0, 0.62);\n                        background-color: white;\n                        overflow: hidden;\n                        z-index: 9999;\n                    }\n\n                    .fg-emoji-container svg {\n                        max-width: 100%;\n                        box-sizing: border-box;\n                        width: 15px;\n                        height: 15px;\n                    }\n\n                    .fg-emoji-picker-category-title {\n                        display: block;\n                        margin: 20px 0 0 0;\n                        padding: 0 10px 5px 10px;\n                        font-size: 16px;\n                        font-family: sans-serif;\n                        font-weight: bold;\n                        flex: 0 0 calc(100% - 20px);\n                        border-bottom: 1px solid #ededed;\n                    }\n\n                    .fg-emoji-nav {\n                        background-color: #646772;\n                    }\n\n                    .fg-emoji-nav li a svg {\n                        transition: all .2s ease;\n                        fill: white;\n                    }\n\n                    .fg-emoji-nav li:hover a svg {\n                        fill: black;\n                    }\n\n                    .fg-emoji-nav ul {\n                        display: flex;\n                        flex-wrap: wrap;\n                        list-style: none;\n                        margin: 0;\n                        padding: 0;\n                        border-bottom: 1px solid #dbdbdb;\n                    }\n\n                    .fg-emoji-nav ul li {\n                        flex: 1;\n                    }\n\n                    .fg-emoji-nav ul li a {\n                        display: flex;\n                        justify-content: center;\n                        align-items: center;\n                        height: 40px;\n                        transition: all .2s ease;\n                    }\n\n                    .fg-emoji-nav ul li a:hover {\n                        background-color: #e9ebf1;\n                    }\n\n                    .fg-emoji-nav ul li.active a {\n                        background-color: #e9ebf1;\n                    }\n\n                    .fg-emoji-nav ul li.emoji-picker-nav-active a {\n                        background-color: #e9ebf1;\n                    }\n\n                    .fg-emoji-nav ul li.emoji-picker-nav-active a svg {\n                        fill: #646772;\n                    }\n\n                    .fg-emoji-picker-move {\n                        /* pointer-events: none; */\n                        cursor: move;\n                    }\n\n                    .fg-picker-special-buttons a {\n                        background-color: ').concat(_this2.options.specialButtons || '#ed5e28', ';\n                    }\n\n                    .fg-picker-special-buttons:last-child a {\n                        box-shadow: inset 1px 0px 0px 0 rgba(0, 0, 0, 0.11);\n                    }\n\n                    .fg-emoji-list {\n                        list-style: none;\n                        margin: 0;\n                        padding: 0;\n                        overflow-y: scroll;\n                        overflow-x: hidden;\n                        height: 323px;\n                    }\n\n                    .fg-emoji-picker-category-wrapper {\n                        display: flex;\n                        flex-wrap: wrap;\n                        flex: 1;\n                    }\n\n                    .fg-emoji-list li {\n                        position: relative;\n                        display: flex;\n                        flex-wrap: wrap;\n                        justify-content: center;\n                        align-items: center;\n                        flex: 0 0 calc(100% / 6);\n                        height: 50px;\n                    }\n\n                    .fg-emoji-list li a {\n                        position: absolute;\n                        width: 100%;\n                        height: 100%;\n                        text-decoration: none;\n                        display: flex;\n                        flex-wrap: wrap;\n                        justify-content: center;\n                        align-items: center;\n                        font-size: 23px;\n                        background-color: #ffffff;\n                        border-radius: 3px;\n                        transition: all .3s ease;\n                    }\n                    \n                    .fg-emoji-list li a:hover {\n                        background-color: #ebebeb;\n                    }\n\n                    .fg-emoji-picker-search {\n                        position: relative;\n                    }\n\n                    .fg-emoji-picker-search input {\n                        border: none;\n                        box-shadow: 0 0 0 0;\n                        outline: none;\n                        width: calc(100% - 30px);\n                        display: block;\n                        padding: 10px 15px;\n                        background-color: #f3f3f3;\n                    }\n\n                    .fg-emoji-picker-search .fg-emoji-picker-search-icon {\n                        position: absolute;\n                        right: 0;\n                        top: 0;\n                        width: 40px;\n                        height: 100%;\n                        display: flex;\n                        align-items: center;\n                        justify-content: center;\n                    }\n\n                </style>\n            ');
      document.head.insertAdjacentHTML('beforeend', e);
    },
    position: function position() {
      const e = (i = window.event).clientX;
      var i = i.clientY;
      const t = {};
      return t.left = e, t.top = i, t;
    },
    rePositioning: function rePositioning(e) {
      e.getBoundingClientRect().right > window.screen.availWidth && (e.style.left = ''.concat(window.screen.availWidth - e.offsetWidth, 'px')), window.innerHeight > 400 && e.getBoundingClientRect().bottom > window.innerHeight && (e.style.top = ''.concat(window.innerHeight - e.offsetHeight, 'px'));
    },
    render: function render(e, i) {
      n = void 0;

      let t = _this2.options.trigger.findIndex((e) => e.selector === i);

      if (_this2.insertInto = _this2.options.trigger[t].insertInto, t = g.position(), !a.length) {
        for (const o in j) {
          if (j.hasOwnProperty.call(j, o)) {
            const l = j[o];
            m += '<li>\n                            <a title="'.concat(o, '" href="#').concat(o, '">').concat(r[o], '</a>\n                        </li>'), a += '<div class="fg-emoji-picker-category-wrapper" id="'.concat(o, '">'), a += '<p class="fg-emoji-picker-category-title">'.concat(o, '</p>'), l.forEach((e) => {
              a += '<li data-title="'.concat(e.title.toLowerCase(), '">\n                                    <a title="').concat(e.title, '" href="#">').concat(e.emoji, '</a>\n                                </li>');
            }), a += '</div>';
          }
        }
      }

      document.querySelector('.fg-emoji-container') && _this2.lib('.fg-emoji-container').remove(), t = '\n                <div class="fg-emoji-container" style="left: '.concat(t.left, 'px; top: ').concat(t.top, 'px;">\n                    <nav class="fg-emoji-nav">\n                        <ul>\n                            ').concat(m, '\n\n                            ').concat(_this2.options.closeButton ? '<li class="fg-picker-special-buttons"><a id="fg-emoji-picker-close-button" href="#">'.concat(s.close, '</a></li>') : '', '\n                        </ul>\n                    </nav>\n\n                    <div class="fg-emoji-picker-search">\n                        <input type="text" placeholder="Search" autofocus />\n                        \n                        <span class="fg-emoji-picker-search-icon">')
        .concat(s.search, '</sapn>\n                    </div>\n\n                    <div>\n                        <!--<div class="fg-emoji-picker-loader-animation">\n                            <div class="spinner">\n                                <div class="bounce1"></div>\n                                <div class="bounce2"></div>\n                                <div class="bounce3"></div>\n                            </div>\n                        </div>-->\n\n                        <ul class="fg-emoji-list">\n                            ')
        .concat(a, '\n                        </ul>\n                    </div>\n                </div>\n            '), document.querySelector('.reply').insertAdjacentHTML('beforebegin', t), g.rePositioning(document.querySelector('.fg-emoji-container')), setTimeout(() => {
        document.querySelector('.fg-emoji-picker-search input').focus();
      }, 500);
    },
    closePicker: function closePicker(e) {
      e.preventDefault(), _this2.lib('.fg-emoji-container').remove(), t = !1;
    },
    checkPickerExist: function checkPickerExist(e) {
      !document.querySelector('.fg-emoji-container') || e.target.closest('.fg-emoji-container') || t || g.closePicker.call(this, e);
    },
    setCaretPosition: function setCaretPosition(e, i) {
      const t = e;
      if (t != null) {
        if (t.createTextRange) {
          const o = t.createTextRange();
          o.move('character', i), o.select();
        } else t.selectionStart ? (t.focus(), t.setSelectionRange(i, i)) : t.focus();
      }
    },
    insert: function insert(e) {
      e.preventDefault(), e = e.target.innerText.trim();
      const i = document.querySelectorAll(_this2.insertInto);
      const o = e;
      i.forEach((e) => {
        let i;
        let t;
        document.selection ? (e.focus(), sel = document.selection.createRange(), sel.text = o) : e.selectionStart || e.selectionStart == '0' ? (i = e.selectionStart, t = e.selectionEnd, e.value = e.value.substring(0, i) + o + e.value.substring(t, e.value.length), g.setCaretPosition(e, i + 2)) : (e.value += o, e.focus());
      });
    },
    categoryNav: function categoryNav(e) {
      e.preventDefault();
      const i = e.target.closest('a');
      if (i.getAttribute('id') && i.getAttribute('id') === 'fg-emoji-picker-close-button') return !1;
      if (i.className.includes('fg-emoji-picker-move')) return !1;
      e = i.getAttribute('href');
      const t = document.querySelector('.fg-emoji-list');
      const o = t.querySelector(''.concat(e));
      _this2.lib('.fg-emoji-nav li').removeClass('emoji-picker-nav-active'), i.closest('li').classList.add('emoji-picker-nav-active'), o.scrollIntoView({
        behavior: 'smooth',
        block: 'start',
        inline: 'nearest',
      });
    ...
    search: function search(e) {
      ...
      n = n || Array.from(document.querySelectorAll('.fg-emoji-picker-category-wrapper li')), n.filter((e) => {
        e.getAttribute('data-title').match(i) ? e.style.display = '' : e.style.display = 'none';
      });
    },
    ...
    mouseMove: function mouseMove(e) {
      if (t) {
        ...
        const _i3 = document.querySelector('.fg-emoji-container');

        _i3.style.left = ''.concat(e.clientX - 320, 'px'), _i3.style.top = ''.concat(e.clientY - 10, 'px');
      }
    },
  };
  g.styles(), function () {
    _this2.lib(document.body).on('click', g.closePicker, '#fg-emoji-picker-close-button'), _this2.lib(document.body).on('click', g.checkPickerExist), _this2.lib(document.body).on('click', g.render, _this2.trigger), _this2.lib(document.body).on('click', g.insert, '.fg-emoji-list a'), _this2.lib(document.body).on('click', g.categoryNav, '.fg-emoji-nav a'), _this2.lib(document.body).on('input', g.search, '.fg-emoji-picker-search input'), _this2.lib(document).on('mousedown', g.mouseDown, '#fg-emoji-picker-move'), _this2.lib(document).on('mouseup', g.mouseUp, '#fg-emoji-picker-move'), _this2.lib(document).on('mousemove', g.mouseMove);
  }.call(this);
}
