const gif = document.getElementById('js-gif__images'); const showBtn = document.getElementById('js-show-more'); const gifSearch = document.querySelector('#js-gif-search'); const searchBtn = document.querySelector('#js-search-btn'); const img = document.querySelector('#js-img'); const showSelection = document.getElementById('js-show-selection'); let showClicked = !1; function loadImage(i, t) {
  ...
  this.options = e, this.trigger = this.options.trigger.map((e) => e.selector), this.insertInto = void 0; let a = ''; let m = ''; let n; let t = !1; const i = this.options.closeButton ? 370 : 350; this.lib = function (t = void 0) {
      ...
      const e = `
                <style>
                    ...
                    .fg-picker-special-buttons a {
                        background-color: ${this.options.specialButtons || '#ed5e28'};
                    }
    ...
    position: () => { var e = window.event; const i = e.clientX; var e = e.clientY; const t = {}; return t.left = i, t.top = e, t; },
    rePositioning: (e) => { e.getBoundingClientRect().right > window.screen.availWidth && (e.style.left = `${window.screen.availWidth - e.offsetWidth}px`), window.innerHeight > 400 && e.getBoundingClientRect().bottom > window.innerHeight && (e.style.top = `${window.innerHeight - e.offsetHeight}px`); },
    render: (e, i) => {
      n = void 0; let t = this.options.trigger.findIndex((e) => e.selector === i); this.insertInto = this.options.trigger[t].insertInto; t = g.position(); if (!a.length) {
        for (const o in j) {
          if (j.hasOwnProperty.call(j, o)) {
            const l = j[o]; m += `<li>
                            <a title="${o}" href="#${o}">${r[o]}</a>
                        </li>`, a += `<div class="fg-emoji-picker-category-wrapper" id="${o}">`, a += `<p class="fg-emoji-picker-category-title">${o}</p>`, l.forEach((e) => {
              a += `<li data-title="${e.title.toLowerCase()}">
                                    <a title="${e.title}" href="#">${e.emoji}</a>
                                </li>`;
            }), a += '</div>';
          }
        }
      }document.querySelector('.fg-emoji-container') && this.lib('.fg-emoji-container').remove(); t = `
                <div class="fg-emoji-container" style="left: ${t.left}px; top: ${t.top}px;">
                    <nav class="fg-emoji-nav">
                        <ul>
                            ${m}

                            ${this.options.closeButton ? `<li class="fg-picker-special-buttons"><a id="fg-emoji-picker-close-button" href="#">${s.close}</a></li>` : ''}
                        </ul>
                    </nav>
                    ...
                    <div class="fg-emoji-picker-search">
                        ...
                        <span class="fg-emoji-picker-search-icon">${s.search}</sapn>
                    </div>
            ...
            `;document.querySelector('.reply').insertAdjacentHTML('beforebegin', t), g.rePositioning(document.querySelector('.fg-emoji-container')), setTimeout(() => { document.querySelector('.fg-emoji-picker-search input').focus(); }, 500);
    },
    closePicker: (e) => { e.preventDefault(), this.lib('.fg-emoji-container').remove(), t = !1; },
    checkPickerExist(e) { !document.querySelector('.fg-emoji-container') || e.target.closest('.fg-emoji-container') || t || g.closePicker.call(this, e); },
    setCaretPosition: (e, i) => { const t = e; if (t != null) if (t.createTextRange) { const o = t.createTextRange(); o.move('character', i), o.select(); } else t.selectionStart ? (t.focus(), t.setSelectionRange(i, i)) : t.focus(); },
    insert: (e) => { e.preventDefault(); e = e.target.innerText.trim(); const i = document.querySelectorAll(this.insertInto); const o = e; i.forEach((e) => { let i; let t; document.selection ? (e.focus(), sel = document.selection.createRange(), sel.text = o) : e.selectionStart || e.selectionStart == '0' ? (i = e.selectionStart, t = e.selectionEnd, e.value = e.value.substring(0, i) + o + e.value.substring(t, e.value.length), g.setCaretPosition(e, i + 2)) : (e.value += o, e.focus()); }); },
    categoryNav: (e) => { e.preventDefault(); const i = e.target.closest('a'); if (i.getAttribute('id') && i.getAttribute('id') === 'fg-emoji-picker-close-button') return !1; if (i.className.includes('fg-emoji-picker-move')) return !1; e = i.getAttribute('href'); const t = document.querySelector('.fg-emoji-list'); const o = t.querySelector(`${e}`); this.lib('.fg-emoji-nav li').removeClass('emoji-picker-nav-active'), i.closest('li').classList.add('emoji-picker-nav-active'), o.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' }); },
    search: (e) => { const i = e.target.value.trim(); n = n || Array.from(document.querySelectorAll('.fg-emoji-picker-category-wrapper li')), n.filter((e) => { e.getAttribute('data-title').match(i) ? e.style.display = '' : e.style.display = 'none'; }); },
    mouseDown: (e) => { e.preventDefault(), t = !0; },
    mouseUp: (e) => { e.preventDefault(), t = !1; },
    mouseMove: (e) => { if (t) { e.preventDefault(); const i = document.querySelector('.fg-emoji-container'); i.style.left = `${e.clientX - 320}px`, i.style.top = `${e.clientY - 10}px`; } },
  }; const o = () => { this.lib(document.body).on('click', g.closePicker, '#fg-emoji-picker-close-button'), this.lib(document.body).on('click', g.checkPickerExist), this.lib(document.body).on('click', g.render, this.trigger), this.lib(document.body).on('click', g.insert, '.fg-emoji-list a'), this.lib(document.body).on('click', g.categoryNav, '.fg-emoji-nav a'), this.lib(document.body).on('input', g.search, '.fg-emoji-picker-search input'), this.lib(document).on('mousedown', g.mouseDown, '#fg-emoji-picker-move'), this.lib(document).on('mouseup', g.mouseUp, '#fg-emoji-picker-move'), this.lib(document).on('mousemove', g.mouseMove); }; g.styles(), o.call(this);
} new Emoji({ trigger: [{ selector: '.emoji-btn', insertInto: ['.textarea'] }], closeButton: !0 });
